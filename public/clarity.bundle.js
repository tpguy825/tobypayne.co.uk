(function () {
	...
	function api(method) {
		return window["Zone"] && "__symbol__" in window["Zone"] ? window["Zone"]["__symbol__"](method) : method;
	}
	var startTime = 0;
	function start$E() {
								...
								switch (key) {
									case "tag":
										...
										if (size && size.length === 2) {
											tokens.push(
												"".concat("#").concat(str$1(size[0]), ".").concat(str$1(size[1])),
											);
										}
										break;
									case "attributes":
		...
		if (observer$2) {
			...
			if (value && value.metadata.size !== null && value.metadata.size.length === 0) {
				...
				if (node && node.nodeType === Node.ELEMENT_NODE) {
					...
					value.metadata.size = [Math.floor(r.width * 100), Math.floor(r.height * 100)];
	...
	function num$2(text, scale, localize) {
		...
		try {
			...
			if (Intl && Intl.NumberFormat && lang && localize) {
				...
				var group = Intl.NumberFormat(lang).format(11111).replace(formatRegex, "");
				var decimal = Intl.NumberFormat(lang).format(1.1).replace(formatRegex, "");
				return Math.round(
					parseFloat(
						text.replace(new RegExp("\\" + group, "g"), "").replace(new RegExp("\\" + decimal), "."),
					) * scale,
				);
			}
	...
	function selector(input, beta) {
		...
		switch (input.tag) {
			...
			default:
				...
				input.tag = input.tag.indexOf("svg:") === 0 ? input.tag.substr("svg:".length) : input.tag;
				var selector = "".concat(prefix).concat(input.tag).concat(suffix);
				var classes = "class" in a && a["class"].length > 0 ? a["class"].trim().split(/\s+/) : null;
				if (beta) {
					...
					selector =
						classes.length > 0
							? "".concat(prefix).concat(input.tag, ".").concat(classes.join(".")).concat(suffix)
							: selector;
					selector =
						id && hasDigits(id) === false ? "".concat(getDomPrefix(prefix), "#").concat(id) : selector;
				} else {
					selector = classes
						? "".concat(prefix).concat(input.tag, ".").concat(classes.join(".")).concat(suffix)
						: selector;
	...
	function getDomPrefix(prefix) {
		...
		var iframeDomStart = prefix.lastIndexOf("".concat("iframe:").concat("HTML"));
		...
		try {
			if (init) {
				config$1.unmask.forEach(function (x) {
					return x.indexOf("!") < 0 ? unmask.push(x) : override.push(x.substr(1));
				});
			}
	...
	function getPrivacy(node, data, parent, privacy) {
		...
		if (tag === "*T" && parent && parent.data) {
			...
			privacy = parent.data.tag === "STYLE" || parent.data.tag === "TITLE" ? 0 : privacy;
		...
		if ("type" in attributes && DISALLOWED_TYPES.indexOf(attributes["type"]) >= 0) {
			privacy = 2;
		}
		...
		if (regionMap.has(node) === false) {
			...
			observer$1 =
				observer$1 === null && watch
					? new IntersectionObserver(handler$2, { threshold: [0, 0.2, 0.4, 0.6, 0.8, 1] })
					: observer$1;
	...
	function track$4(id, event) {
		var node = getNode(id);
		var data =
			id in regions ? regions[id] : { id: id, visibility: 0, interaction: 16, name: regionMap.get(node) };
		var interaction = 16;
		switch (event) {
			case 9:
				interaction = 20;
		...
		for (var _i = 0, entries_1 = entries; _i < entries_1.length; _i++) {
			...
			if (
				...
				var data =
					id in regions
						? regions[id]
						: { id: id, name: regionMap.get(target), interaction: 16, visibility: 0 };
				var viewportRatio = overlap
					? (overlap.width * overlap.height * 1) / (viewport.width * viewport.height)
					: 0;
				var visible = viewportRatio > 0.05 || entry.intersectionRatio > 0.8;
				var scrolledToEnd =
					(visible || data.visibility == 10) && Math.abs(rect.top) + viewport.height > rect.height;
				process$6(target, data, data.interaction, scrolledToEnd ? 13 : visible ? 10 : 0);
	...
	function clone(r) {
		return {
			time: time(),
			data: { id: r.id, interaction: r.interaction, visibility: r.visibility, name: r.name },
		};
	}
	function reset$f() {
		state$7 = [];
	...
	function recompute$7(action, evt) {
		state$5.push({ time: time(), event: 38, data: { target: target(evt), action: action } });
	...
	function recompute$5() {
		...
		data$8 = {
			...
			height:
				de && "clientHeight" in de ? Math.min(de.clientHeight, window.innerHeight) : window.innerHeight,
		};
		encode$2(11);
	}
	function reset$a() {
	...
	function recompute$4(event) {
		...
		var current = { time: time(), event: 10, data: { target: element, x: x, y: y } };
	...
	var interaction = Object.freeze({ __proto__: null, start: start$f, stop: stop$e, observe: observe$3 });
	...
	function ld(json) {
		for (var _i = 0, _a = Object.keys(json); _i < _a.length; _i++) {
			...
			if (key === "@type" && typeof value === "string") {
				...
				value = value.indexOf("article") >= 0 || value.indexOf("posting") >= 0 ? "article" : value;
				switch (value) {
					case "article":
					case "recipe":
						log(5, json[key]);
		...
		switch (node.nodeType) {
			...
			case Node.DOCUMENT_FRAGMENT_NODE:
				...
				if (shadowRoot.host) {
					...
					if (type === "function" && shadowRoot.constructor.toString().indexOf("[native code]") >= 0) {
						observe$2(shadowRoot);
						var style = "";
						var adoptedStyleSheets =
							"adoptedStyleSheets" in shadowRoot ? shadowRoot["adoptedStyleSheets"] : [];
						for (
						...
						dom[call](node, shadowRoot.host, { tag: "*P", attributes: {} }, source);
					}
				}
				break;
			case Node.TEXT_NODE:
			...
			case Node.ELEMENT_NODE:
				...
				switch (tag) {
					case "HTML":
						...
						break;
					case "SCRIPT":
						if ("type" in attributes && attributes["type"] === "application/ld+json") {
							try {
					...
					case "NOSCRIPT":
						break;
					case "META":
						var key = "property" in attributes ? "property" : "name" in attributes ? "name" : null;
	...
	function track$2(time, event, hash, x, y, reaction, context) {
		...
		state.push({
			time: time,
			event: 22,
			data: { type: event, hash: hash, x: x, y: y, reaction: reaction, context: context },
		});
		track$7(event, x, y);
	}
	function compute$4() {
	...
	function stringify(encoded) {
		return encoded.p.length > 0
			? '{"e":'.concat(encoded.e, ',"a":').concat(encoded.a, ',"p":').concat(encoded.p, "}")
	...
	function delay() {
		var gap = config$1.lean === false && discoverBytes > 0 ? 100 : data$1.sequence * config$1.delay;
		return typeof config$1.upload === "string" ? Math.max(Math.min(gap, 3e4), 100) : config$1.delay;
	}
	...
	function start$9() {
		...
		config$1.lean = config$1.track && s.upgrade !== null ? s.upgrade === 0 : config$1.lean;
		config$1.upload =
			config$1.track &&
	...
	function save() {
		...
		var upload =
			config$1.upload && typeof config$1.upload === "string" ? config$1.upload.replace("https://", "") : "";
		if (upgrade && callback) {
			callback(data$2, !config$1.lean);
		}
		setCookie("_clsk", [data$2.sessionId, ts, data$2.pageNum, upgrade, upload].join("|"), 1);
	}
	...
	function track(u, consent) {
		...
		if (u.expiry === null || Math.abs(end - u.expiry) >= 1 || u.consent !== consent) {
			setCookie("_clck", [data$2.userId, 1, end.toString(36), consent].join("|"), 365);
		}
	}
	function shortid() {
	...
	function setCookie(key, value, time) {
		if (config$1.track && ((navigator && navigator.cookieEnabled) || supported(document, "cookie"))) {
			...
			var cookie = "".concat(key, "=").concat(value).concat(";").concat(expires).concat(";path=/");
			try {
				if (rootDomain === null) {
					var hostname = location.hostname ? location.hostname.split(".") : [];
					for (var i = hostname.length - 1; i >= 0; i--) {
						rootDomain = ".".concat(hostname[i]).concat(rootDomain ? rootDomain : "");
						if (i < hostname.length - 1) {
							document.cookie = "".concat(cookie).concat(";").concat("domain=").concat(rootDomain);
	...
	function reset$1() {
		for (var _i = 0, bindings_1 = bindings; _i < bindings_1.length; _i++) {
			...
			try {
				binding.target[api("removeEventListener")](binding.event, binding.listener, binding.capture);
			} catch (_a) {}
		}
		bindings = [];
	}
	...
	var diagnostic = Object.freeze({ __proto__: null, start: start$5, stop: stop$4 });
	function start$4() {
	...
	var observer;
	var types = ["navigation", "resource", "longtask", "first-input", "layout-shift", "largest-contentful-paint"];
	function start$2() {
	...
	var performance$1 = Object.freeze({ __proto__: null, start: start$1, stop: stop$1 });
